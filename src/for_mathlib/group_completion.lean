...
lemma same_uniformity : topological_add_group.to_uniform_space (completion H) = uniform_space.completion_uniform_space H:=
sorry
...
-- Note that the uniform space structure chosen by Lean for completion H is
-- `topological_add_group.to_uniform_space (completion H)`
...
begin
  ...
  have hom : is_add_group_hom (to_completion H ∘ f), by apply_instance,
  have := same_uniformity H,
  letI := topological_add_group.to_uniform_space (completion H),

  haveI complete : complete_space (completion H) := begin
    -- Why can't I do something like `rw same_uniformity H at _inst,`?
    sorry
  end,
